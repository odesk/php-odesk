...
```
{
    ...
    "require": {
        "odesk/php-odesk": "v0.1.16" // note: the latest release is recommended
    }
}
```
...
Updating dependencies (including require-dev)
  - Installing odesk/php-odesk (v0.1.16)
    Downloading: 100%         
...
let's say it is https://github.com/odesk/php-odesk/archive/v0.1.16.tar.gz, and
...
```
require_once __DIR__ . '/odesk/php-odesk-0.1.16/src/oDesk/API/constants.php';

spl_autoload_register('oDeskVendorAutoloader');

function oDeskVendorAutoloader($_class)
{
    $path = __DIR__ . '/odesk/php-odesk-0.1.16/src/' . str_replace('\\', '/', $_class) . '.php';
    include_once $path;
}
```
